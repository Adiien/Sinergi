...
<head>
    ...
    <title>Explore Forums - SINERGI</title>
...
<body class="bg-[#eff3f8] h-screen flex flex-col overflow-hidden">
        ...
        <aside class="w-80 bg-white flex flex-col h-full z-10 hidden lg:flex rounded-tr-[30px] shadow-sm relative">
            <div class="p-8 flex flex-col h-full">
                ...
                <nav class="space-y-2 mb-8">
                    ...
                    <a href="<?= BASE_URL ?>/forum" class="flex items-center px-2 py-2 text-gray-600 hover:text-[#1e293b] font-bold transition group">
                ...
                <div class="flex items-center justify-between mb-4 border-t pt-4 border-gray-100">
                    ...
                    <a href="<?= BASE_URL ?>/forum" class="text-xs text-blue-600 font-semibold hover:underline">See All</a>
                </div>

                <div class="space-y-4 overflow-y-auto custom-scroll pr-2 flex-1">
                    <?php if (!empty($joinedForums)): ?>
                        <?php foreach ($joinedForums as $jf): ?>
                            <a href="<?= BASE_URL ?>/forum/show?id=<?= $jf['FORUM_ID'] ?>" class="flex items-center space-x-3 cursor-pointer hover:bg-gray-50 p-2 rounded-xl transition group">
                                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-lg shadow-sm shrink-0">
                                    <?php if (!empty($jf['COVER_IMAGE'])): ?>
                                        <img src="<?= BASE_URL ?>/public/uploads/forums/<?= $jf['COVER_IMAGE'] ?>" class="w-full h-full object-cover rounded-xl">
                                    <?php else: ?>
                                        <?= substr($jf['NAME'], 0, 1) ?>
                                    <?php endif; ?>
                                </div>
                ...
                <div id="view-forum-list" class="fade-in">
                    <div class="flex justify-between items-end mb-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">Discover new communities to join.</p>
                        </div>
                        <a href="<?= BASE_URL ?>/forum" class="text-sm font-bold text-gray-500 hover:text-blue-600 hover:underline flex items-center transition">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        <?php if (!empty($forums)): ?>
                            <?php
                            // Ambil ID forum yang sudah diikuti untuk cek status
                            $joinedIds = array_column($joinedForums ?? [], 'FORUM_ID');
                            ?>
                            <?php foreach ($forums as $forum):
                                $isJoined = in_array($forum['FORUM_ID'], $joinedIds);
                            ?>
                                <div class="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col h-[300px]">
                                    <div class="h-36 relative overflow-hidden bg-gray-200">
                                        ...
                                        <div class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-lg text-[10px] font-bold text-gray-700 shadow-sm">
                                            <?= isset($forum['VISIBILITY']) ? ucfirst($forum['VISIBILITY']) : 'Public' ?>
                                        </div>
                                    </div>

                                    <div class="p-5 flex-1 flex flex-col justify-between">
                                        <div>
                                            <h3 class="font-bold text-lg text-gray-800 leading-tight mb-1 truncate" title="<?= htmlspecialchars($forum['NAME']) ?>">
                                            ...
                                            <p class="text-xs text-gray-500 flex items-center">
                                                ...
                                                <?= number_format($forum['MEMBER_COUNT'] ?? 0) ?> Members
                                            </p>
                                            <p class="text-xs text-gray-400 mt-2 line-clamp-2">
                                                <?= !empty($forum['DESCRIPTION']) ? htmlspecialchars($forum['DESCRIPTION']) : 'No description available.' ?>
                                            </p>
                                        </div>

                                        <div class="mt-4">
                                            <?php if ($isJoined): ?>
                                                <a href="<?= BASE_URL ?>/forum/show?id=<?= $forum['FORUM_ID'] ?>" class="block text-center w-full border border-gray-200 bg-gray-50 text-gray-600 font-bold py-2 rounded-xl hover:bg-gray-100 transition text-sm">
                                                    View Forum
                                                </a>
                                            <?php else: ?>
                                                <form action="<?= BASE_URL ?>/forum/show?id=<?= $forum['FORUM_ID'] ?>" method="POST">
                                                    <input type="hidden" name="forum_id" value="<?= $forum['FORUM_ID'] ?>">
                                                    <button type="submit" class="block w-full border border-transparent bg-blue-600 text-white font-bold py-2 rounded-xl hover:bg-blue-700 shadow-md hover:shadow-lg transition text-sm">
                                                        Open Community
                                                    </button>
                                                </form>
                                            <?php endif; ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="col-span-full flex flex-col items-center justify-center py-20 text-gray-400 bg-white rounded-[30px] border border-dashed border-gray-200">
                                <svg class="w-16 h-16 text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                                </svg>
                                <p class="text-lg font-medium">No forums found matching your criteria.</p>
                                <a href="<?= BASE_URL ?>/forum/explore" class="text-sm text-blue-600 font-bold mt-2 hover:underline">Clear Search</a>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
                    ...
                    <div class="flex justify-center">
                        <div class="bg-white rounded-[30px] shadow-sm border border-gray-100 p-10 w-full max-w-2xl relative">
                            ...
                            <form action="<?= BASE_URL ?>/forum/create" method="POST">
                                ...
                                <div class="mb-8">
                                    <textarea name="description" class="w-full border border-gray-300 rounded-lg px-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-blue-500 transition text-gray-800 text-sm font-medium h-32 resize-none" placeholder="Describe your forum..."></textarea>
                                </div>
                                <div class="mb-10">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            // Logic Tab Create Forum vs List
            ...
            // Logic Sidebar Search (Copy-Paste from index.php with endpoint check)
            ...
            document.addEventListener('click', (e) => {
                if (!searchContainer.contains(e.target)) searchDropdown.classList.add('hidden');
            });
            searchInput.addEventListener('focus', () => {
                if (searchInput.value.trim().length > 0) searchDropdown.classList.remove('hidden');
            });
        });
    </script>
