document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderNotifItem(notif) {
    ...
    if (notif.TYPE === "like") {
      message = `<span class="font-bold">${notif.ACTOR_NAME}</span> liked your post.`;
      ...
      message = `<span class="font-bold">${notif.ACTOR_NAME}</span> commented on your post.`;
    ...
    // Background lebih gelap jika belum dibaca
    const bgClass = notif.IS_READ == 0 ? "bg-blue-50/50" : "bg-white";

    return `
      <a href="${BASE_URL}/home" class="flex items-start space-x-3 px-4 py-3 hover:bg-gray-50 transition border-b border-gray-50 ${bgClass}">
        <div class="flex-shrink-0 mt-1">
           ${icon}
        </div>
        <div class="flex-1 min-w-0">
           <p class="text-sm text-gray-800 break-words">${message}</p>
           <p class="text-xs text-gray-400 mt-1">${notif.FMT_TIME}</p>
        </div>
        ${
          notif.IS_READ == 0
